...
class RTLearner(object):
    ...
    def build_tree(leaf_size, data_x, data_y):
        ...
        if len(np.unique(data_y)) == 1:
            ...
            lefttree = RTLearner.build_tree(leaf_size, data_x[data_x[:, i] <= SplitVal], data_y[data_x[:, i] <= SplitVal])
            righttree = RTLearner.build_tree(leaf_size, data_x[data_x[:, i] > SplitVal], data_y[data_x[:, i] > SplitVal])
    ...
    def add_evidence(self, data_x, data_y):
        ...
        self.regression_tree = RTLearner.build_tree(self.leaf_size, data_x, data_y)
    ...
    def traverse_tree(point, tree_data, tree_index=0):
        ...
        if point[factor] <= SplitVal:
            return RTLearner.traverse_tree(point, tree_data, tree_index=left_ind)
        else:
            return RTLearner.traverse_tree(point, tree_data, tree_index=right_ind)
    ...
    def query(self, points):
        ...
        :rtype: numpy.ndarray
        """
        result = np.apply_along_axis(RTLearner.traverse_tree, axis=1, arr=points, tree_data=self.regression_tree, tree_index=0)
        return result
